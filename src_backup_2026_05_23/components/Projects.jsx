...
const Projects = () => {
  ...
  const [viewMode, setViewMode] = useState(() => {
    return localStorage.getItem('projectsViewMode') || 'bookshelf';
  });
  ...
  return (
    <div className="tab-projects proj-root">
      {/* Header Intelligence - Absolute Floating */}
      <div className="proj-filter-wrapper">
        <div className="proj-filter-tools">
          {/* View Switcher */}
          <NeumorphicViewSwitcher viewMode={viewMode} setViewMode={setViewMode} />
        ...
        <div>
          ...
          <div className="w-full">
            {viewMode === 'grid' ? (
                ...
                <AnimatePresence mode="popLayout">
                  {filteredProjects.map((project, idx) => (
                    <motion.div
                      ...
                      className="proj-grid-card group"
                      style={{ '--proj-color': project.color || '#0078d4' }}
                    >
                      {/* Top: Image/Logo Area (2/3 height) */}
                      <div className="proj-grid-image-area">
                        {project.image ? (
                          <img 
                      ...
                      <div 
                        ...
                        style={{ '--proj-bg-color': project.color || '#1e293b' }}
                      >
                        <div className="proj-grid-details-inner">
                          <div className="flex gap-2 items-start leading-[14px]">
                            <span className="w-[100px] text-[11px] font-black uppercase shrink-0">NAME :</span>
                            <span className="text-[11px] font-bold truncate">{project.name || 'N/A'}</span>
                          </div>
                          
                          <div className="flex gap-2 items-start leading-[14px]">
                            <span className="w-[100px] text-[11px] font-black uppercase shrink-0">CREATED AT :</span>
                          ...
                          <div className="flex gap-2 items-start leading-[14px]">
                            <span className="w-[100px] text-[11px] font-black uppercase shrink-0">VERSION :</span>
                            <span className="text-[11px] font-bold">Autodesk Revit {project.revit_version || '2024'}</span>
                          </div>

                          <div className="flex gap-2 items-start leading-[14px]">
                            <span className="w-[100px] text-[11px] font-black uppercase shrink-0">DESCRIPTION :</span>
                        ...
                        <div className="absolute bottom-[5px] right-[5px]">
                          <div className="px-2 py-1 bg-white/20 backdrop-blur-md rounded-[4px] font-black uppercase tracking-widest shadow-sm flex items-center gap-1">
                            <span className="text-[24px] leading-none">{project.taskCount || 0}</span>
                            <span className="text-[9px] pt-1">Tasks</span>
                          </div>
                        </div>
                      </div>
                    </motion.div>
              ...
              <div className="proj-shelf-wrapper group/shelf">
                <div className="proj-shelf-bg-gradient" />
                  ...
                  {filteredProjects.length > 0 ? (
                    filteredProjects.map((project, idx) => (
                      <motion.div
                        ...
                        onClick={() => setSelectedId(project.id)}
                        className="proj-book-item group/book"
                        style={{
                          '--book-w': `${[42, 50, 58, 46, 54, 62, 38][idx % 7]}px`,
                          '--book-h': `${280 + (Math.abs(idx % 8 - 4) * 18)}px`,
                          '--book-color': project.color || '#1e293b',
                        }}
                      >
                ...
                <div className="proj-shelf-base" />
                <div className="proj-vault-banner vault-banner group-hover/shelf:opacity-100">
                  <span className="vault-banner-text">Corporate Project Intelligence Vault</span>
                </div>
              </div>
            )}
          </div>
        ...
        {selectedId && selectedProject && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 md:p-12 overflow-y-auto" style={{ perspective: 1200 }}>
            ...
            <motion.div
              ...
              className="proj-modal-wrapper"
              style={{ '--proj-color': selectedProject.color || '#3b82f6' }}
            >
