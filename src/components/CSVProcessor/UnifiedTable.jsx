    ...
    <div className="bg-[var(--bg-card)] border border-[var(--border)] shadow-md rounded-2xl m-[20px] min-w-[calc(100%-40px)] w-fit overflow-hidden">
        <table className="w-full text-left border-separate border-spacing-0" style={{ minWidth: '1500px' }}>
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {tableRows.length === 0 ? (
              <tr><td colSpan={16} className="sys-px sys-py text-center text-[var(--text-muted)] font-bold uppercase text-sm border-b border-[var(--border)] py-8">No tasks found</td></tr>
            ) : tableRows.map((r, i) => {
              const rowBg = 'bg-transparent';
              const stickyBg = 'bg-[var(--bg-card)]';
